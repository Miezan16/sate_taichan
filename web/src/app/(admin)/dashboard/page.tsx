  ...
  const handleImageUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      if (file.size > 1024 * 1024) {
        alert(
          "Ukuran gambar terlalu besar! Maksimal 1 MB. Mohon kompres gambar Anda terlebih dahulu."
        );
        return;
      }
              ...
              <div className="flex items-center gap-4 bg-white/5 border border-white/10 p-3 rounded-xl transition-all hover:bg-white/10">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-[10px] text-gray-500 mt-1">
                    Sistem otomatis menyimpan gambar
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="flex items-center justify-between bg-white/5 border border-white/10 p-4 rounded-xl mt-2">
              <div>
                ...
                <p className="text-xs text-gray-500">
                  Tampilkan menu ini di kasir / pelanggan
                </p>
              </div>
              <label className="relative inline-flex items-center cursor-pointer">
                <input
                  ...
                  onChange={(e) =>
                    setFormData({ ...formData, tersedia: e.target.checked })
                  }
                />
...
function OverviewTab({
  ...
  const bestSellers = useMemo(() => {
    ...
    transactions.forEach((t) => {
      if (t.items && Array.isArray(t.items)) {
        t.items.forEach((item) => {
          const menuName = item.menu?.nama || "Menu Tidak Dikenal";
    ...
    const sortedSellers = Object.entries(menuSales)
  ...
  if (!analyticsData) {
    return (
      <div className="flex flex-col items-center justify-center h-full">
        <Loader2 size={40} className="animate-spin opacity-50 text-violet-400" />
        <p className="text-gray-400 mt-4">Memuat data analitik...</p>
      </div>
    );
  }
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="bg-gradient-to-br from-violet-500/10 to-purple-900/20 border border-violet-500/20 p-5 rounded-2xl relative overflow-hidden group hover:bg-violet-500/10 transition-colors">
          ...
          <div className="flex items-center gap-3 mb-3 relative z-10">
            ...
            <h3 className="text-violet-400/80 text-xs font-bold uppercase tracking-wider">
              Pendapatan Bulanan
            </h3>
          </div>
          <p className="text-2xl font-black text-violet-300 relative z-10">
            {formatRp(revenueMonth)}
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 h-[400px]">
        <div className="lg:col-span-2 bg-white/5 border border-white/10 p-5 rounded-2xl flex flex-col h-full">
          <h3 className="text-sm font-bold text-gray-300 mb-4 flex items-center gap-2">
            <Activity size={16} className="text-violet-400" /> Grafik Tren
  ...
  value >= 1000 ? `Rp${value / 1000}k` : `Rp${value}`
        ...
        <div className="bg-white/5 border border-white/10 p-5 rounded-2xl flex flex-col h-full">
          <h3 className="text-sm font-bold text-gray-300 mb-4 flex items-center gap-2">
            <TrendingUp size={16} className="text-emerald-400" /> Peringkat
            Penjualan Menu
          </h3>
          <div className="flex-1 overflow-y-auto space-y-2 pr-2 [&::-webkit-scrollbar]:w-1 [&::-webkit-scrollbar-thumb]:bg-white/10">
            {bestSellers.length > 0 ? (
              bestSellers.map((item, index) => (
                <div
                  key={item.name}
                  className="flex justify-between items-center p-3 bg-black/20 border border-white/5 rounded-xl hover:bg-white/5 transition-colors"
                >
    ...
    const rowsHtml = transactions
      .map((h) => {
        const branchName =
          ...
          branches.find((b) => b.id === h.cabang_id)?.nama_cabang ||
          "-";
        return `
      <tr>
      ...
      <div className="p-6 border-b border-white/10 flex justify-between items-center bg-white/[0.02]">
        <div>
          <h2 className="font-bold text-lg text-white flex items-center gap-2">
            <History className="text-cyan-400" size={20} /> Riwayat Transaksi
            Kasir
          </h2>
        </div>
        <button
        ...
        <table className="w-full text-left text-sm">
          <thead className="bg-black/40 text-gray-400 text-[10px] uppercase sticky top-0 z-10 backdrop-blur-md border-b border-white/10">
            <tr>
              ...
              <th className="p-5 font-bold tracking-wider">Pelanggan / Meja</th>
              <th className="p-5 font-bold tracking-wider">Kasir</th>
              <th className="p-5 font-bold tracking-wider">Cabang</th>
              <th className="p-5 font-bold tracking-wider">Metode</th>
          ...
          <tbody className="divide-y divide-white/5">
            {transactions.map((t) => {
              const branchName =
                ...
                branches.find((b) => b.id === t.cabang_id)?.nama_cabang ||
                "-";
              return (
                <tr key={t.id} className="hover:bg-white/5 transition-colors">
                  ...
                  <td className="p-4">
                    ...
                    <span className="ml-2 bg-white/10 px-2 py-0.5 rounded text-[10px] font-bold text-gray-400">
                      Meja {t.nomor_meja}
                    </span>
                  </td>
                  ...
                  <td className="p-4 text-emerald-400 font-medium text-xs">
                    {branchName}
                  </td>
                  <td className="p-4">
                    <span
...
function MenuTab({
  ...
  // HARD DELETE - Menghapus menu dari database
  const handleDelete = async (menu: MenuItem) => {
    try {
      ...
      if (res.ok) {
        alert(`Menu "${menu.nama}" telah dihapus dari database!`);
        onRefresh();
        setConfirm(null);
      } else {
  ...
  return (
    <div className="h-full flex flex-col space-y-4">
      <div className="flex justify-between items-center">
        ...
        <div className="flex gap-3">
          ...
          <button
            ...
            className="flex items-center gap-2 bg-gradient-to-r from-cyan-500 to-blue-600 px-5 py-2.5 rounded-xl text-sm font-bold hover:shadow-lg hover:shadow-cyan-500/20 transition-all"
          >
      ...
      {confirm && (
        <ConfirmModal
          message={`Hapus menu "${confirm.nama}" dari database? Tindakan ini TIDAK DAPAT dibatalkan. Menu akan hilang permanen dari kasir dan database.`}
...
function UsersTab({
  ...
  const roleBadge = (role: string) =>
    role === "admin" ? (
      <span className="inline-flex items-center gap-1 px-2 py-1 rounded-lg text-[10px] font-bold bg-violet-500/10 text-violet-400 border border-violet-500/30">
        <ShieldCheck size={10} /> Admin
      </span>
    ) : (
      <span className="inline-flex items-center gap-1 px-2 py-1 rounded-lg text-[10px] font-bold bg-cyan-500/10 text-cyan-400 border border-cyan-500/30">
    ...
    <div className="h-full flex flex-col space-y-4">
      <div className="flex justify-end">
        <button
          ...
          className="flex items-center gap-2 bg-gradient-to-r from-violet-500 to-purple-600 px-5 py-2.5 rounded-xl text-sm font-bold hover:shadow-lg hover:shadow-violet-500/20 transition-all"
        >
          ...
          <tbody className="divide-y divide-white/5">
            {users.map((u) => (
              <tr key={u.id} className="hover:bg-white/5 transition-colors">
                <td className="p-4">
                  <div className="flex items-center gap-3">
                    <div className="w-8 h-8 rounded-full bg-gradient-to-br from-violet-500 to-purple-600 flex items-center justify-center text-white font-black text-xs">
  ...
  return (
    <div className="h-full flex flex-col bg-white/5 border border-white/10 rounded-2xl overflow-hidden backdrop-blur-md">
      <div className="p-6 border-b border-white/10 bg-white/[0.02]">
        <h2 className="font-bold text-lg text-white flex items-center gap-2">
          <Activity className="text-cyan-400" size={20} /> Log Aktivitas Sistem
        </h2>
      </div>
      <div className="flex-1 overflow-y-auto [&::-webkit-scrollbar]:w-2 [&::-webkit-scrollbar-thumb]:bg-white/10">
        <table className="w-full text-left text-sm">
...
export default function AdminDashboard() {
  ...
  return (
    <div className="flex h-screen bg-black text-white font-sans overflow-hidden">
      <aside className="w-20 bg-black/40 border-r border-white/5 flex flex-col items-center py-6 z-20">
        ...
        <header className="px-8 py-5 flex justify-between items-center border-b border-white/5 bg-black/20 backdrop-blur-sm z-10">
          <div>
            ...
            <p className="text-gray-500 text-[11px] mt-0.5">
              Admin Panel - Pantau keseluruhan sistem dan data restoran
            </p>
          </div>
          ...
          {loading && menus.length === 0 ? (
            ...
            <AnimatePresence mode="wait">
              ...
              >
                ...
                {activeTab === "history" && (
                  <HistoryTab transactions={transactions} branches={branches} />
                )}
                {activeTab === "menu" && (
                  <MenuTab menus={menus} onRefresh={fetchAll} />
                )}
                {activeTab === "users" && (
                  <UsersTab
                    users={users}
